...
function ENT:Use(activator, caller)
	if self:IsLit() && IsValid(activator) && activator:IsPlayer() then
		setLight(self, false)
	end
end
...
function setLight(ent, state, noFire)
	...
	if errorFunc(!IsValid(ent), "Light-Entity is missing") then
		return
	end
	...
	if ent:IsLit() then
		// Play ignite sound
		ent:EmitSound("sunabouzu/candle_light.wav", 100, 100)
